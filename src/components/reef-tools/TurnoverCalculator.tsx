...
export default function TurnoverCalculator() {
  ...
  const tabCls = (active: boolean) => `flex-1 py-2 rounded-lg text-xs font-medium text-center transition-all ${active ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200 border border-slate-700'}`;
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-white font-semibold text-sm">Flow / Turnover Rate</h3>
          <p className="text-slate-500 text-xs">Check if your flow is right for your reef type</p>
        </div>
      </div>
      ...
      <p className="text-xs text-slate-500 mb-4">{cfg.desc} — {cfg.min}–{cfg.max}× turnover recommended</p>

      <div className="space-y-3">
        <div>
          <label className="text-xs text-slate-400 mb-1.5 block">Tank Volume (gallons)</label>
        ...
        {recMin && recMax && (
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 flex items-center justify-between">
            <span className="text-xs text-slate-300">Recommended flow</span>
        ...
        <div>
          <label className="text-xs text-slate-400 mb-1.5 block">Your Total Flow (GPH)</label>
        ...
        {actualTurnover && status && (
          <div className={`rounded-xl p-3 border ${statusConfig[status].bg} flex items-center justify-between`}>
            <div>
              ...
              <div className="text-xs text-slate-400 mt-0.5">{actualTurnover}× turnover per hour</div>
            </div>
