  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 transition-colors duration-200">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-slate-900 dark:text-white font-semibold text-sm">Water Change Calculator</h3>
          <p className="text-slate-400 dark:text-slate-500 text-xs">Volume and parameter dilution planning</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1.5 block">Tank Volume (gal)</label>
            <input className={inputCls} placeholder="100" value={tankVol} onChange={e => setTankVol(e.target.value)} type="number" min="0" />
          </div>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1.5 block">Change Size (%)</label>
        ...
        {wc && (
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-slate-100/60 dark:bg-slate-800/60 rounded-xl p-3 text-center">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">gallons to change</div>
            </div>
            <div className="bg-slate-100/60 dark:bg-slate-800/60 rounded-xl p-3 text-center">
              <div className="text-xl font-bold text-teal-400">{wc.liters.toFixed(1)}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">liters to change</div>
            </div>
          </div>
        )}

        <div className="border-t border-slate-200 dark:border-slate-800 pt-3">
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 font-medium">Parameter dilution estimate</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-500 dark:text-slate-400 mb-1.5 block">Current ({paramDefaults[paramType].unit})</label>
            ...
            <div>
              <label className="text-xs text-slate-500 dark:text-slate-400 mb-1.5 block">Target ({paramDefaults[paramType].unit})</label>
          ...
          {nc && (
            <div className="mt-3 bg-sky-500/10 border border-sky-500/20 rounded-xl p-3 flex items-center justify-between">
              <span className="text-sm text-slate-700 dark:text-slate-300">Water changes needed</span>
