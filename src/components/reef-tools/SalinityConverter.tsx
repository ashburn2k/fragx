...
export default function SalinityConverter() {
  ...
  const tabCls = (active: boolean) => `flex-1 py-2 rounded-lg text-xs font-medium transition-all ${active ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200 border border-slate-700'}`;
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-white font-semibold text-sm">Salinity Converter</h3>
          <p className="text-slate-500 text-xs">Convert between SG, PPT, and refractometer</p>
        </div>
      </div>
      ...
      <div>
        <label className="text-xs text-slate-400 mb-1.5 block">{labels[mode]}</label>
        <input
          className={inputCls}
      ...
      {res && (
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className={`rounded-xl p-3 text-center ${mode === 'sg' ? 'bg-cyan-500/10 border border-cyan-500/20' : 'bg-slate-800/60'}`}>
            <div className="text-lg font-bold text-cyan-400">{res.sg.toFixed(4)}</div>
            <div className="text-xs text-slate-400 mt-0.5">SG</div>
          </div>
          <div className={`rounded-xl p-3 text-center ${mode === 'ppt' ? 'bg-cyan-500/10 border border-cyan-500/20' : 'bg-slate-800/60'}`}>
            <div className="text-lg font-bold text-teal-400">{res.ppt.toFixed(2)}</div>
            <div className="text-xs text-slate-400 mt-0.5">PPT</div>
          </div>
          <div className={`rounded-xl p-3 text-center ${mode === 'refractometer' ? 'bg-cyan-500/10 border border-cyan-500/20' : 'bg-slate-800/60'}`}>
            <div className="text-lg font-bold text-sky-400">{res.ref.toFixed(2)}</div>
            <div className="text-xs text-slate-400 mt-0.5">Refract.</div>
          </div>
        </div>
      )}
