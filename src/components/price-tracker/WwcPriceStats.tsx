...
export default function WwcPriceStats({ products, collection }: WwcPriceStatsProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 space-y-4 transition-colors duration-200">
      <div>
        <h3 className="text-slate-900 dark:text-white font-semibold capitalize">{collection.replace(/-/g, ' ')} — Price Summary</h3>
        <p className="text-slate-400 dark:text-slate-500 text-xs mt-0.5">{products.length} products listed on WorldWideCoral</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 text-center transition-colors duration-200">
          <div className="text-slate-900 dark:text-white font-bold text-xl">${avg.toFixed(0)}</div>
          <div className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">Avg Price</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 text-center transition-colors duration-200">
          <div className="text-emerald-400 font-bold text-xl">${min.toFixed(0)}</div>
          <div className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">Lowest</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 text-center transition-colors duration-200">
          <div className="text-red-400 font-bold text-xl">${max.toFixed(0)}</div>
          <div className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">Highest</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 text-center transition-colors duration-200">
          <div className="text-amber-400 font-bold text-xl">{onSale}</div>
          <div className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">On Sale</div>
        </div>
      </div>

      <div>
        <p className="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider mb-3">Price Distribution</p>
        <div className="space-y-1.5">
          {buckets.map(bucket => (
            <div key={bucket.label} className="flex items-center gap-3">
              <span className="text-slate-500 dark:text-slate-400 text-xs w-20 shrink-0">{bucket.label}</span>
              ...
              <span className="text-slate-400 dark:text-slate-500 text-xs w-6 text-right">{bucket.count}</span>
            </div>
          ))}
        </div>
      </div>
