    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-white">WWC Live Prices</h1>
            <a
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className={`p-2 rounded-xl border transition-all duration-200 ${showStats ? 'border-cyan-500 text-cyan-400 bg-cyan-900/20' : 'border-slate-700 text-slate-400 hover:text-white'}`}
      ...
      {products.length === 0 && !loading && !scraping && (
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-10 text-center">
          <RefreshCw size={36} className="mx-auto mb-3 text-slate-600" />
          <p className="text-white font-semibold mb-1">No price data yet</p>
          <p className="text-slate-400 text-sm mb-4">Click "Refresh Prices" to scrape the latest prices from WorldWideCoral.</p>
          <button
            onClick={handleScrape}
            ...
            {(['coral', 'fish'] as const).map(tab => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-medium transition-all duration-200 capitalize ${
                  activeTab === tab ? 'bg-cyan-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                }`}
              >
          ...
          {showFilters && (
            <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs text-slate-400 mb-1.5">Collection</label>
