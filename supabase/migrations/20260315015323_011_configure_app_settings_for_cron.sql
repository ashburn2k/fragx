/*
  ...
  Stores the project's Supabase URL and anon key as session-level settings
  accessible to the trigger_weekly_vendor_scrape() cron function.
  Uses SET LOCAL inside a DO block as an alternative approach.

  Since ALTER DATABASE is restricted, we embed the values directly in the
  cron function instead of relying on app.settings.
*/
