/*
  # Fix RLS Policy Auth Initialization Performance

  ## Summary
  Replaces all direct `auth.uid()` and `auth.jwt()` calls in RLS policies with
  `(select auth.uid())` subquery pattern. This ensures the auth function is called
  once per query instead of once per row, significantly improving performance at scale.

  ## Tables Fixed
  1. profiles - insert, update policies
  2. user_badges - insert policy
  3. listings - select, insert, update, delete policies
  4. listing_images - insert, delete policies
  5. listing_tags - insert, delete policies
  6. price_history - insert policy
  7. have_list - insert, update, delete policies
  8. want_list - insert, update, delete policies
  9. trade_matches - select, insert, update policies
  10. messages - select, insert, update policies
  11. reviews - insert policy
  12. flags - select, insert policies
  13. price_alerts - select, insert, update, delete policies
*/
