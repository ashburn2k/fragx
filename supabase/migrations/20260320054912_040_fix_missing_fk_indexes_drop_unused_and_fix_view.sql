/*
  # Fix Missing FK Indexes, Drop Unused Indexes, and Fix View Security

  ## Summary
  Addresses all outstanding security and performance advisor warnings.

  ## 1. Drop Unused Indexes
  The following indexes exist but have never been used by the query planner.
  Dropping them reduces write overhead and storage without any read performance loss.
  - idx_listings_seller_id (listings)
  - idx_listings_species_id (listings)
  - idx_have_list_user_id (have_list)
  - idx_want_list_user_id (want_list)
  - idx_price_history_morph_id (price_history)
  - idx_price_history_species_id (price_history)
  - idx_messages_recipient_id (messages)
  - idx_messages_sender_id (messages)

  ## 2. Add Missing Foreign Key Indexes
  All 24 foreign key columns that lack a covering index.
  These improve JOIN and ON DELETE CASCADE performance significantly.

  ## 3. Fix coral_products View Security
  Recreates the coral_products view with security_invoker = true so it
  runs under the calling user's permissions rather than the view owner's,
  eliminating the SECURITY DEFINER warning.
*/
...
-- ============================================================
-- FIX coral_products VIEW: SECURITY INVOKER
-- ============================================================
