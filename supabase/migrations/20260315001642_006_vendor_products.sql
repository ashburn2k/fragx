/*
  ...
  ## Overview
  Generic multi-vendor product price storage. Each vendor is identified by a slug
  (e.g. "unique-corals", "vivid-aquariums"). Products are scraped from Shopify
  collection JSON endpoints and stored here for in-app price comparison.

  ## New Tables

  ### vendor_scrape_configs
  - `id` (uuid, primary key)
  - `slug` (text, unique) - machine-readable vendor slug e.g. "unique-corals"
  - `name` (text) - display name e.g. "Unique Corals"
  - `base_url` (text) - Shopify store root e.g. "https://uniquecorals.com"
  - `coral_collections` (text[]) - collection slugs to scrape for corals
  - `fish_collections` (text[]) - collection slugs to scrape for fish
  - `is_active` (boolean) - whether this vendor should be scraped
  - `created_at` (timestamptz)

  ### vendor_products
  - `id` (uuid, primary key)
  - `vendor_slug` (text) - which vendor this product belongs to
  - `shopify_id` (bigint) - Shopify product ID (unique per vendor)
  ...
  - Public read for all (price data is public)
