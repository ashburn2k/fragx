...
async function scrapeVendor(
  ...
  // --- Full catalog via /products.json (catches everything) ---
  if (vendor.use_products_endpoint !== false) {
    ...
    for (const product of products) {
      ...
      const record = buildRecord(product, vendor.slug, collection);
      if (!record) continue;
  ...
  // --- Collection-based scrape (supplements /products.json with proper collection labels) ---
  const collections = [
  ...
  for (const collection of collections) {
    ...
    while (true) {
      ...
      for (const product of products) {
        const record = buildRecord(product, vendor.slug, collection);
        if (!record) continue;
        // First-seen collection wins — specialty collections run before catch-alls
        // so their labels are preserved when a product appears in multiple collections
  ...
  // --- Track price changes and build history ---
  ...
  // --- Mark unavailable products (scraped before but not seen now) ---
